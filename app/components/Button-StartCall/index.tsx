...
const StartCallButton = ({ currentCharacter }: ActiveCallProps) => {
  ...
  return (
    <div>
      <button
        onClick={() => handleTalkToSanta('Talk To Santa')}
        className="text-white text-sm tracking-wide whitespace-nowrap justify-center items-center border border-[color:var(--black,#000)] bg-teal-800 self-stretch mt-4 px-16 py-5 rounded-3xl border-solid max-md:px-5"
      >
        Talk to Santa
      </button>
      <a
        onClick={() => handleCallMe('Call Me!')}
        href="tel:1234567890"
