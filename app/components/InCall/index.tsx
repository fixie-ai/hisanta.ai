...
const InCall = ({ currentCharacter }: ActiveCallProps) => {
  ...
  return (
    <div>
      <div className="w-[275px] h-[493px]">
      <form onSubmit={(event) => event.preventDefault()} className="border border-[color:var(--slate-800,#1E293B)] shadow-xl bg-white bg-opacity-80 flex flex-col items-center px-4 py-5 rounded-[36px] border-solid">
      <header className="text-red-900 text-center text-xl tracking-normal whitespace-nowrap">
        {currentCharacter.name}
      </header>
      <Image src={`/images/${currentCharacter.image}`} alt={`${currentCharacter.name} Image.`} width={300} height={300} className="aspect-[0.85] object-contain object-center w-[170px] overflow-hidden max-w-full mt-7" />    
